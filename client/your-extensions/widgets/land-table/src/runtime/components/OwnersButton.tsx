...
/** - Tlačítko pro zobrazení vlastníků. */
export default function({ landSearchState, tableRef }: HSI.LandTableWidget.IOwnerButton) {
...
function OwnersButton({ queryResponse, tableRef }: Pick<HSI.LandTableWidget.IOwnerButton, "tableRef"> & { queryResponse: HSI.UseLandQueries.IExecuteQueryResponse; }) {
    ...
    async function showOwners() {
        try {
            ...
            const response = await RequestHelper.jsonRequest<HSI.UseLandQueries.IExecuteQueryResponse>(`${layers[0].url}/exts/${LayerHelper.getExtensionValue(EKnownLayerExtension.SdSoe)}/Pozemky/GetOwners`, {
    ...
    return <>
        ...
        >
            <ModalHeader toggle={() => setState(defaultState)}>{messageFormater("ownersModalHeader")}</ModalHeader>
            <ModalBody className={!state.isModalOpen || state.loadStatus !== ELoadStatus.Loaded ? "widget-land-hidden-table" : ""}>
                {
                    function() {
                        ...
                        if (state.loadStatus === ELoadStatus.Error) {
                            return <WarningContent message={state.errorMessage} title={messageFormater("failedToLoadOwners")} />;
                        }
                        ...
                        if (state.loadStatus === ELoadStatus.Loaded && state.response.DataRows.length < 1) {
                            return <div style={{ padding: 10 }}>
                                <CalciteNotice
                                    ...
                                    kind="info"
                                >
                                    <div slot="message">{messageFormater("noOwnerFound")}</div>
                                </CalciteNotice>
                            </div>;
                        }
            ...
            <ModalFooter>
                <ButtonGroup size="sm">
                    <Button onClick={() => setState(defaultState)}>
                        {messageFormater("closeOwnerModalButton")}
                    </Button>
                </ButtonGroup>
            </ModalFooter>
        </Modal>
