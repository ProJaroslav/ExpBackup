...
/** - Zobrazení negrafické relační třídy prvku ve stromové struktuře. */
export default function(props: IRelationshipItemProps) {
    ...
    const onContextMenu = useRelationClassContextMenu(relations, itemReference, true);
                ...
                return <>
                    {
                        relations.result.features.map(feature => {
                            let gisId = FeatureHelper.getTableFeatureGisId(feature);
                            return <TableFeatureItem
                                isRelation
                                key={gisId}
                                feature={feature}
